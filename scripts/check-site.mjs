...
for (const densityStyle of ["Compact atlas density", ".hero { min-height: 560px", ".quest-card { min-height: 148px", ".atlas-tree a { min-height: 38px"]) assert.ok(css.includes(densityStyle), `Missing compact atlas contract: ${densityStyle}`);
...
for (const densityStyle of ["Compact reading density", ".reader-article{padding:48px", "line-height:1.62", ".code-panel code{line-height:1.58", ".practice-actions button,.trace-controls button{min-height:36px"]) assert.ok(bookCss.includes(densityStyle), `Missing compact reader contract: ${densityStyle}`);
